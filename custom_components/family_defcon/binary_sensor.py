...
class MutualDestructionBinarySensor(BinarySensorEntity):
    """Report whether Mutual WiFi Destruction is active."""

    _attr_name = "Mutual WiFi Destruction"
    _attr_unique_id = "family_defcon_mutual_wifi_destruction"
    _attr_suggested_object_id = "family_defcon_mutual_wifi_destruction"
    ...
    @property
    def is_on(self) -> bool:
        return bool(self.hass.data[DOMAIN]["state"]["mutual_destruction"])
...
class ParentAdminConfirmedBinarySensor(BinarySensorEntity):
    ...
    def is_on(self) -> bool:
        ...
        return (
            bool(state.get("parent_admin_confirm"))
